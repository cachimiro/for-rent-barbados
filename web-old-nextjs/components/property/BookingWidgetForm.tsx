...
interface BookingWidgetFormProps {
  property: Property;
  checkIn: string;
  checkOut: string;
...
export default function BookingWidgetForm({
  property,
  checkIn,
  checkOut,
  onCheckInChange,
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${SUPABASE_URL}/functions/v1/availability${qs}`, {
        signal: controller.signal,
      }).then((r) => r.json()),
      fetch(`${SUPABASE_URL}/functions/v1/pricing${qs}`, {
        signal: controller.signal,
      }).then((r) => r.json()),
    ])
      .then(([avail, pricing]) => {
  ...
  const bookingUrl = (() => {
    ...
    params.set("slug", property.slug);
    if (checkIn) params.set("checkIn", checkIn);
    if (checkOut) params.set("checkOut", checkOut);
    if (guests) params.set("guests", String(guests));
    return `https://forrentbarbados.com/booking-confirmation/?${params.toString()}`;
  })();
  ...
  return (
    ...
    >
      ...
      {/* CTA */}
      <a
        href={isAvailable === false ? undefined : bookingUrl}
        style={{
          display: "block",
          background: isAvailable === false ? "#aaa" : "#1a1a1a",
          color: "#FFFFFF",
          fontFamily: "var(--font-poppins), sans-serif",
          fontSize: 13,
          fontWeight: 400,
      ...
      >
        {!hasPropertyId ? "Contact to Book" : "Confirm Reservation"}
      </a>
